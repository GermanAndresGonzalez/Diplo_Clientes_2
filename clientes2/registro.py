...
class RegistroLogError(Exception):
    ...
    def __init__(self, linea, modulo, fecha, usuario=None, *args):
        ...
        self.modulo = modulo
        self.fecha = fecha  # self.fecha = fecha.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def registrar(self):
        ...
        print("Registrando error...", self.fecha, self.linea)
        datos = [
            self.fecha,
            self.linea,
            self.modulo,
        ...
        print(mensaje)
        self.sock.sendto(
            bytes(" ".join(map(str, datos)), "utf-8"), (self.HOST, self.PORT)
        )
        recibido = self.sock.recv(1024).decode("utf-8")
        print(recibido)
